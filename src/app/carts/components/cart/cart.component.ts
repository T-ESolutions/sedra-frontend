...
export class CartComponent implements OnInit {
  ...
  deleteCart(cartId: number) {
    this.spinner.show();
    this._CartsService.deleteCart(cartId).subscribe(
      (response) => {
        if (response.status == 200) {
          this.spinner.hide();
          this.getProductOnCart();
        ...
        if (response.status == 200) {
          ...
          if (this.checkLogin == 'true') {
            this.finalTotalDiscount =
              this.cart.discount + this.userData.discount;
          } else {
          ...
          if (this.checkLogin == 'true') {
            this.finalTotalPrice =
              this.cart.total -
              ((this.userData.discount / 100) * this.cart.total +
                (this.cart.discount / 100) * this.cart.total +
                this.cart.shipping);
          } else {
            this.finalTotalPrice =
              this.cart.total -
  ...
  sendValues() {
    ...
    sessionStorage.setItem(
      'finalTotalDiscount',
      JSON.stringify(this.finalTotalDiscount)
    );
    sessionStorage.setItem(
      'finalTotalPrice',
      JSON.stringify(this.finalTotalPrice)
    );
    sessionStorage.setItem('copoun', JSON.stringify(this.coupon_code));
    if (this.checkLogin == 'true') {
      sessionStorage.setItem(
        'finalTotalShipping',
        JSON.stringify(this.userData.shipping_free)
      );
    } else {
  ...
  submitCopounForm(copounForm: FormGroup) {
    ...
    if (copounForm.invalid) {
      ...
      this._CartsService.applyCopoun(copounForm.value).subscribe(
        (response) => {
          if (response.status == 200) {
            ...
            this.finalTotalDiscount =
              (response.data.discount / response.data.sub_total) * 100;
            this.finalTotalPrice =
              this.cart.total -
              (this.finalTotalDiscount / 100) * this.cart.total;
