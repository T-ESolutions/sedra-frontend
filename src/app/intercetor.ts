    ...
    catchError((error: HttpErrorResponse) => {
       ...
       if (error.error instanceof ErrorEvent) {
          ...
          if(error.status == 401 ) {
           ...
           else if (error.status != 422 && error.status != 401 && error.status != 406) {
            Swal.fire({
              title: 'خطأ !!',
          ...
          else if( error.status == 422 && error.error.errors.coupon_code[0] == "القيمة المحددة كوبون الخصم غير موجودة.") {
            Swal.fire({
              title: 'خطأ !!',
